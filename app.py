...
class UltimateResume(FPDF):
    def add_page_border(self, color):
        # Page ke charo taraf border lagane ke liye
        self.set_draw_color(*color)
        self.set_line_width(1.0)
        # 5mm ka margin chod kar rectangle draw kiya hai
...
def create_pdf(data, color_theme, photo_file):
    ...
    if photo_file:
        ...
        photo_path = os.path.join(temp_dir, "sonu_border.jpg")
    ...
    pdf.add_page_border(color) # Border yahan add kiya hai
    
    return pdf.output(dest='S').encode('latin-1', 'ignore')

# --- Streamlit UI ---
st.set_page_config(page_title="AI Resume Maker Pro", layout="wide")
st.title("💎 AI Resume Builder (With Border)")
...
if st.button("Generate Final Resume"):
    ...
    st.download_button("📥 Download PDF", data=pdf_out, file_name=f"{name}_Resume_Border.pdf")
